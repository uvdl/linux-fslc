...
static struct sensor_data adv7610_data;
...
{
	const struct sensor_data *sensor = &adv7610_data;
...
static int ioctl_g_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	...
	sensor->pix.height = fmt.height;
	sensor->pix.width = fmt.width;
	sensor->streamcap.timeperframe.denominator = fmt.fps;
...
{
	struct sensor_data *sensor = &adv7610_data;
	...
	sensor->streamcap.timeperframe.denominator = 0;
	sensor->streamcap.timeperframe.numerator = 0;
	sensor->pix.width = 0;
	sensor->pix.height = 0;
#ifdef USE_16BIT
